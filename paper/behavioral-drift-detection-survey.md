...
A second extension is an observability ladder. The corpora of Section 7 contain richer layers than the boundary records we scored — plain-text call parameters, tool outputs, reasoning text, token counts; our adapters hashed those away by design, the sources did not. Repeating the same experiments with the same ground truth and splits, one added layer at a time, would price each rung: whether content-level features lift the measured invisibility of reward hacking (the Terminal Wrench authors report explicit justification in the reasoning trace for ~72% of exploit attempts, exactly one rung above where Section 7.5 measured 0%), and how much of TRAIL's unannotated ~94% becomes visible with tool outputs in view. The condition of Section 6 carries up the ladder: each rung's detectors must stay deterministic — pattern and structure features over stored content, not LLM judgment — or the evidentiary property this paper argues for is spent at the second rung.
